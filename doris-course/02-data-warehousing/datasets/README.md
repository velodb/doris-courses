...
| 数据 | 来源及用途 | 口径 |
| --- | --- | --- |
| [wwi/sample.json](wwi/sample.json) | WWI 十笔历史订单及关联维度；Module 1–4 | 税前订单金额 12220.60，不表示收款 |
| [wwi/wwi-core.tar.gz](wwi/wwi-core.tar.gz) | Module 5：10 张表、701,846 行，解压后约 16.2 MiB | 原始订单、明细、客户、商品、发票、客户账款及字典 |
| orders.json / orders.csv | 课程生成的十笔新订单；Module 5、Module 6 | 初始金额 1400.00，支付和退款均为零 |
| raw_orders.json | 10 笔正常新订单 + 非法金额、空订单号、无效客户各一行 | 13 输入 → 10 合格、3 拒收 |
| malformed_orders.csv | 一行正常、一行非法金额 | Module 5 严格模式整批拒绝 |
| orders_reordered.csv | Module 5 独立练习：两笔订单，客户号在订单号之前 | 显式配置导入列映射，金额合计 260.00 |
...
Module 1–3 小样本、模拟事件与 Module 5 完整 Parquet 压缩包都随仓库提供。
Module 5 首次读取时将约 10 MiB 的压缩包解到课程目录 `.runtime/wwi/`，
校验十个文件的大小与 SHA-256 后开始导入。已有文件保留，校验不一致时停止并提示差异。
Module 6、Module 7 继续读取 Module 5 导入的客户和商品维度，因此应先完成 Module 5，再进入质量与状态实验。
