...
| D06-02：暂存与拒收 | 输入与分流结果 | 8 分钟 | 追踪原始字段和每条拒收原因 |
| D06-03：质量测试与验收 | 错误注入案例 | 7 分钟 | 证明检查能发现已知错误 |
| 实验 6 | 动手操作 | 25 分钟 | 完成 13→10＋3 分流，生成 D07 合格输入 |
...
分类规则把它标记为 INVALID_AMOUNT；合法金额则保留转换后的数值。
...
`orders_classified` 是保存分类 SQL 的普通视图，查询它时会执行转换、关联和判断。
