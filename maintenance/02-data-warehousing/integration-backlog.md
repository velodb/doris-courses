...
以下内容在 Level 1 仅通过讲义、配置示例和 Quiz 讲解，不要求交付外部实验环境。真实位点恢复讲原理；持续并发验证属于进阶内容。下表保留未实测边界，供将来选择制作集成实验时使用，不是当前课程的待办或结课门槛。
...
Level 1 的学习完成条件为七个主线 Lab、独立练习及 Quiz，三份扩展 Notebook 选做。介绍型内容无需外部链路运行记录；不能把主线通过说成真实持续接入、位点恢复或生产并发已验证。视频录制状态另行记录。
